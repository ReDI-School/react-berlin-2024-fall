/**
 * Lesson 3 - Javascript you need to know for React (object/array destructuring, spread, rest)
 */

import Lesson3ArrayFuncs from "./Lesson3ArrayFuncs";

function Lesson3SpreadRest() {
    ...
    const { name, internalOrgans: { brain: { thought }, stomach: { contents: stomachContents }, intestines: { contents: intestinesContents } } } = objectosaurus
    
    // 2. Count the amount of keys in our foe's, with using less characters 
    const amountOfKeys = [...stomachContents, ...intestinesContents].length
    
    // 3. Find the first and second keys in the foe's digestive tract, in one line
    const [first, second, ...remainder] = [...stomachContents, ...intestinesContents]
    
    
    // 4. Using object destructuring and restructuring, defeat the foe in a single line of code with your sword!
    const doneosaurus = {...objectosaurus, alive: false, struck: "sword", name: "Doneosaurus"}
    ...
    const [, keyA, keyB] = [...stomachContents, ...intestinesContents]
    ...
    if (doneosaurusIsAlive) {
        ...
        return <>
            ...
            {(tryOpeningChest(keyA, keyB, doneosaurus) === "🟩🗝+⬜🗝🔓: Doneosaurus is deceased - struck by a sword") ? <span>Yay! You found: <Lesson3ArrayFuncs /></span> : <span>The chest won&apos;t budge ☹</span>}
        </>
    }
}

export default Lesson3SpreadRest;
